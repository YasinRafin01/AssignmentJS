...
function updateCheckInOutText() {
    ...
    //timeElement1.textContent = selectedCheckOut ? formatDate(selectedCheckOut)+selectedDateRangeOption : 'Check out';
...
document.querySelectorAll('.date-range-option').forEach(option => {
    option.addEventListener('click', (e) => {
        ...
        // selectedDateRangeOption1=e.target.getAttribute('date-days');
        if(!selectedCheckOut){
                // selectedDateRangeOption = e.target.getAttribute('data-days');
                updateCheckInOutText();
        }else if(selectedCheckOut){
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateGuestInputPlaceholder() {
        ...
        let placeholderText = `${totalGuests} ${totalGuests === 1 ? 'guest' : 'guests'}`;
        if (infants > 0) {
...
function updateIconState(isSaved) {
    if (isSaved) {
        ...
        saveIcon.textContent = '♡'; // Empty heart
    }
}
